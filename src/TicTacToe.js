		...
		try {
			...
			collector.on('collect', async (button) => {
				if (button.user.id !== opponent.id)
					return button.reply({
						content: '❌ You cant play the game as they didnt call you to play.',
						ephemeral: true
					})
				...
				if (button.customId == 'declinettt') {
					...
					let epm = new Discord.MessageEmbed()
						.setTitle(options.embedTitle || 'Tic Tac Toe')
						.setColor(options.embedColor || 0x075fff)
						.setFooter(foot)
						.setTimestamp(options.timestamp || true)
